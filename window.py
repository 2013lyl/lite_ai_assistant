...
class SettingWindow(QDialog):
    ...
    def init_ui(self):
        ...
        self.model_name_edit = QLineEdit(model_name)
        self.model_name_edit.setFont(self.setting_text_font)
        self.layout.addWidget(self.model_name_edit)

        self.ollama_base_url_label = QLabel("Ollama本地部署网址")
        self.ollama_base_url_label.setFont(self.setting_text_font)
        self.layout.addWidget(self.ollama_base_url_label)

        self.ollama_base_url_tip_label = QLabel("如：http://localhost:11434/v1")
        self.ollama_base_url_tip_label.setFont(self.tip_font)
        self.layout.addWidget(self.ollama_base_url_tip_label)

        self.ollama_base_url_edit = QLineEdit(ollama_base_url)
        self.ollama_base_url_edit.setFont(self.setting_text_font)
        self.layout.addWidget(self.ollama_base_url_edit)
    ...
    def save_settings(self):
        ...
        new_ollama_data_settings = {}

        new_model_data_settings['model_name'] = self.model_name_edit.text()
        new_ollama_data_settings['ollama_base_url'] = self.ollama_base_url_edit.text()

        rw_data.write_data(new_model_data_settings, new_ollama_data_settings)
...
class Window(QWidget):
    ...
    def read_settings(self):
        global model_name, ollama_base_url

        result_data = rw_data.return_data()

        model_name = result_data['model_name']
        ollama_base_url = result_data['ollama_base_url']
